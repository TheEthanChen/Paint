...
let make_control (v: 'a) : 'a value_controller =
    let value_controller = { contents = (v, [])} in
    { add_change_listener =
        (fun (z : 'a -> unit) ->
          begin match value_controller.contents with
          | (x, y) -> value_controller.contents <- (x, z::y)
          end);
      get_value = 
        (fun () -> 
          begin match value_controller.contents with
      ...
      set_value = 
        (fun (z: 'a) -> 
          begin match value_controller.contents with
          | (x, y) -> (value_controller.contents <- (z, y); 
                       List.iter (fun f -> f z) y)
          end)
  }
...
let checkbox (init: bool) (s: string)
    ...
    let (w_text, h_text) = Gctx.text_size s in
    let bool_controller = make_control init in
    { repaint = (fun (g: Gctx.gctx) ->
        Gctx.draw_rect g (0, h_text) (h_text, h_text);
        Gctx.draw_string g (h_text,0) s;
        if bool_controller.get_value () then 
        Gctx.fill_rect g (0, h_text) (h_text, h_text) else 
        Gctx.draw_rect g (0, h_text) (h_text, h_text));
      handle = (fun _ (e: Gctx.event) -> 
        if (fst e) = Gctx.MouseDown then 
        bool_controller.set_value (not (bool_controller.get_value ())));
      size = (fun () -> (w_text + h_text, h_text))
    }, bool_controller
...
let slider (slider_value: int) (start: int) (ending_val: int) : 
  widget * int value_controller =
  let int_controller = make_control slider_value in
  let current_mouse_pos = {contents = 0} in
    { repaint = (fun (g: Gctx.gctx) ->
        (*  Set slider full width and slider indicator  *)
        Gctx.draw_rect g (0, 20) (50, 20);
        Gctx.fill_rect g (start, 20) (!current_mouse_pos, 20));
      handle = (fun (g: Gctx.gctx) (e: Gctx.event) -> 
        (*  Preview if mouse dragging, and set value when mouse up  *)
        if (fst e) = Gctx.MouseDrag then 
        current_mouse_pos.contents <- fst (Gctx.event_pos e g);
        if (fst e) = Gctx.MouseUp then
        current_mouse_pos.contents <- fst (Gctx.event_pos e g);
        int_controller.set_value (!current_mouse_pos));
      size = (fun () -> (50, 20));
    }, int_controller
